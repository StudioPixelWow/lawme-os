...
if (mode === "scaffold") {
  ...
  for (const e of manifest.entries) {
    const path = `${GT}/${e.id}.json`;
    if (!existsSync(path)) { missing.push(e.id); continue; }
    const gt = JSON.parse(readFileSync(path, "utf8"));
    if (!gt.reading_order_text || !gt.confirmed_stratum || !gt.reviewed_by) { missing.push(`${e.id} (incomplete)`); continue; }
    hashes[e.id] = sha(JSON.stringify(gt));
  }
  if (missing.length) { process.stderr.write(`cannot freeze — ${missing.length} entries missing/incomplete:\n${missing.join("\n")}\n`); process.exit(1); }
  ...
  process.stdout.write(`FROZEN v1: ${manifest.entries.length} pages · combined ${frozen.combined_sha256.slice(0, 16)}…\n`);
} else {
