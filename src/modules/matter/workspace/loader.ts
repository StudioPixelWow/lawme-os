/**
 * Matter Workspace — authorized read layer (Capability 2, Slice 2.0.0). SERVER-ONLY.
 *
 * Reads ONLY existing Capability-1 data through the AUTHENTICATED (RLS) client,
 * gated by the SAME resource-authorization policy as the Matter room: the
 * `matter.read` decision is evaluated BEFORE any content is loaded, so an
 * inaccessible matter is indistinguishable from a missing one (no enumeration).
 *
 * This does NOT touch Bootstrap, the frozen room module, or `hydrateMatter`.
 * It composes the persisted rows the Bootstrap engine already writes — matter
 * header, facts, participants (+contacts), documents, evidence, deadlines, and
 * the human-readable `matter_activity` feed — into the pure presenter input.
 */
...
export async function loadMatterWorkspace(
  ...
  // 2) Resolve the matter header (canonical uuid + display columns).
  ...
  // 4) Resolve display names for linked contacts and profiles (best effort; RLS
  //    may withhold — an absent name honestly renders "לא ידוע", never invented).
  ...
  // 5) Map persisted rows → the pure presenter input.
  ...
  const input: WorkspaceInput = {
    ...
    participants: participantRows.map((p) => {
      ...
      return {
        id: p.id, role: p.role, nameHe: c?.name_he ?? null, kind: c?.kind ?? null,
  ...
  return { ok: true, view: buildWorkspaceView(input) };
}
